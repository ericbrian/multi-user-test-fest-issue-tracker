...
const SCHEMA =
  (process.env.DB_SCHEMA || "testfest").replace(/[^a-zA-Z0-9_]/g, "") ||
  "testfest";
...
const ENTRA_REDIRECT_URI =
  process.env.ENTRA_REDIRECT_URI || "http://localhost:3000/auth/callback";
const GROUPIER_EMAILS = (process.env.GROUPIER_EMAILS || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const TAGS = (process.env.TAGS || "duplicate,as-designed,low-priority")
  .split(",")
  .map((s) => s.trim());
...
async function setupOIDC() {
  ...
  if (!ENTRA_ISSUER || !ENTRA_CLIENT_ID || !ENTRA_CLIENT_SECRET) {
    console.warn(
      "Entra ID OIDC not fully configured. Set ENTRA_ISSUER, ENTRA_CLIENT_ID, ENTRA_CLIENT_SECRET."
    );
    return;
  }
...
app.use(
  createDevAutoAuthMiddleware({ DISABLE_SSO, DEV_USER_EMAIL, DEV_USER_NAME })
);
